...
from services.task_service_supabase import (
...
async def get_tasks(current_user: User = Depends(get_current_user)):
    ...
    tasks_with_info = await get_tasks_with_user_info()
...
async def create_daily_tasks_endpoint(
    ...
    return await create_daily_tasks(current_user.id, task_date)
...
async def update_task_info(
    ...
    task = await update_task(task_id, task_update)
    if not task:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Task not found"
